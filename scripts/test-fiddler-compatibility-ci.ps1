...
The already-built release executable. The bridge is extracted from it without rebuilding.

.PARAMETER ExpectedExecutableSha256
The executable digest recorded by the release-producing job, checked before execution.
...
param(
    ...
    [Parameter(Mandatory)][ValidatePattern('\A[A-Fa-f0-9]{64}\z')][string]$ExpectedExecutableSha256,
...
if (Get-Process -Name Fiddler, fiddler-classic-cli -ErrorAction SilentlyContinue) {
...
$ownedNames = @('FiddlerClassicCLI.Bridge.dll', 'FiddlerClassicCLI.Protocol.dll', 'FiddlerClassicCLI.CompatibilityProbe.dll')
...
& "$PSScriptRoot/verify-release.ps1" -ReleaseDirectory $ReleaseDirectory -ExpectedSha256 $ExpectedExecutableSha256
...
try {
    ...
    & "$PSScriptRoot/expand-test-payload.ps1" -Executable "$ReleaseDirectory/fiddler-classic-cli.exe" `
        -Destination $payload -ExpectedSha256 $ExpectedExecutableSha256
    $bridge = Join-Path $payload 'bridge/FiddlerClassicCLI.Bridge.dll'
    ...
    & dotnet build "$PSScriptRoot/../tests/FiddlerClassicCLI.CompatibilityProbe/FiddlerClassicCLI.CompatibilityProbe.csproj" `
    ...
    $cli = Join-Path $payload 'fiddler-classic-cli.exe'
    $bridgeInstalled = $true
    & $cli bridge install | Out-Null
    if ($LASTEXITCODE -ne 0) { throw 'Bridge deployment in the disposable account failed.' }
    Copy-Item -LiteralPath "$probeOutput/FiddlerClassicCLI.CompatibilityProbe.dll" -Destination $scriptsDirectory
    ...
    # Stop only processes belonging to this test. Do not kill by process name or reset the profile.
    if ($nativeProcess) {
    ...
    try {
        ...
        $probePath = Join-Path $scriptsDirectory 'FiddlerClassicCLI.CompatibilityProbe.dll'
