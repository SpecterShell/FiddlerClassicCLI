...
try {
    ...
    & dotnet build "$PSScriptRoot/../tests/FiddlerClassicCLI.ApiCompatibility/FiddlerClassicCLI.ApiCompatibility.csproj" `
        --configuration Release --output "$testRoot/check" --nologo
    if ($LASTEXITCODE -ne 0) { throw 'API compatibility checker build failed.' }
    & "$testRoot/check/FiddlerClassicCLI.ApiCompatibility.exe" "$payload/bridge/FiddlerClassicCLI.Bridge.dll" `
