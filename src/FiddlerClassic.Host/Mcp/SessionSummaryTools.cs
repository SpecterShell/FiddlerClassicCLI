...
internal sealed class SessionSummaryTools
{
    ...
    [Description("Summarize one bounded page of captured network-request metadata. Reports scope, limit, truncation, host/status counts, captured body bytes (not wire bytes), and available completed timings in milliseconds with median and nearest-rank p95. No header or body reads or searches, no pagination, and no outgoing network requests. Totals cover only returned requests; filters may match more.")]
