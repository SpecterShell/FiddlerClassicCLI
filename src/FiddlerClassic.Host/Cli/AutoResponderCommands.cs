...
internal static class AutoResponderCommands
{
    ...
    private static Command CreateAdd(CliActions actions, Option<bool> jsonOption)
    {
        ...
        latency.Validators.Add(result => ValidateRange(result, 0, ProtocolConstants.MaxBreakpointHoldMilliseconds, "Latency"));
    ...
    private static Command CreateUpdate(CliActions actions, Option<bool> jsonOption)
    {
        ...
        latency.Validators.Add(result =>
        {
            ...
            if (value.HasValue && (value.Value < 0 || value.Value > ProtocolConstants.MaxBreakpointHoldMilliseconds))
            {
                result.AddError($"Latency must be between 0 and {ProtocolConstants.MaxBreakpointHoldMilliseconds}.");
            }
        });
    ...
    private static Command CreateLoad(CliActions actions, Option<bool> jsonOption)
    {
        ...
        var path = new Argument<string>("path");
        var replace = Flag("--replace", "Replace all current rules instead of importing.");
        var yes = Flag("--yes", "Confirm replacement without prompting.", "-y");
        command.Arguments.Add(path);
    ...
    private static void ValidateRange(OptionResult result, int minimum, int maximum, string label)
    {
        var value = result.GetValueOrDefault<int>();
