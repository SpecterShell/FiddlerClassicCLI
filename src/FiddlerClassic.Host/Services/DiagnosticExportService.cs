...
internal sealed class DiagnosticExportService
{
    ...
    /// <param name="outputPath">An explicit absolute file path; stdout, devices, and alternate streams are rejected.</param>
    ...
    /// <summary>Rejects device aliases and Windows path forms that can target streams instead of new regular files.</summary>
    /// <param name="outputPath">The user-supplied destination; never included in validation failures.</param>
    /// <returns>A normalized absolute file path.</returns>
    private static string ValidateOutputPath(string outputPath)
    {
        const string message = "Diagnostic output must be an absolute regular file path, not stdout, a device, or an alternate stream.";
        try
        {
