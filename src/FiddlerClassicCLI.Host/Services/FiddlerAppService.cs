...
internal sealed class FiddlerAppService
{
    ...
    /// <summary>Starts Fiddler only if no target is running. A successful launch does not imply bridge readiness.</summary>
    /// <param name="executablePath">An optional exact installation path. Shell commands are not accepted.</param>
    ...
    /// <param name="cancellationToken">Cancels waiting without killing Fiddler or starting a replacement.</param>
        ...
        int timeoutSeconds, bool confirmed, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            RequireSupported(target.Info);
            // Validate the relaunch file before closing so a missing executable or mismatched
            // path leaves the running instance intact.
