...
internal sealed class SessionFilterOptions
{
    ...
    public void AddTo(Command command)
    {
        ...
        {
            bool errorsOnly, successfulOnly;
            int? minId, maxId;
            double? minDuration, maxDuration;
            long? minBody, maxBody;
            int bodySearchBytes;
            try
            {
                errorsOnly = result.GetValue(_errorsOnly);
                successfulOnly = result.GetValue(_successfulOnly);
                minId = _includeIdBounds ? result.GetValue(_minId) : null;
                maxId = _includeIdBounds ? result.GetValue(_maxId) : null;
                minDuration = result.GetValue(_minDuration);
                maxDuration = result.GetValue(_maxDuration);
                minBody = result.GetValue(_minBodyBytes);
                maxBody = result.GetValue(_maxBodyBytes);
                bodySearchBytes = result.GetValue(_bodySearchBytes);
            }
            catch (InvalidOperationException)
            {
                // Leave conversion failures to the parser before checking relationships between values.
                return;
            }

            if (errorsOnly && successfulOnly)
            {
