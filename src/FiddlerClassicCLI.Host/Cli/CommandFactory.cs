...
internal static class CommandFactory
{
    ...
    /// <summary>
    /// Requires values for supplied data options and shows help for groups without invoking a child action.
    /// </summary>
    /// <param name="command">The command whose descendants receive shared usage behavior.</param>
    private static void ConfigureUsage(Command command)
    {
        foreach (var option in command.Options)
        {
            // Nullable and array options may be omitted. A supplied flag requires a value.
            if (option.ValueType != typeof(bool) && option.Arity.MinimumNumberOfValues == 0
                && option.Arity.MaximumNumberOfValues > 0)
            {
                option.Arity = new ArgumentArity(1, option.Arity.MaximumNumberOfValues);
            }
        }

        if (command.Subcommands.Count > 0 && command.Action is null)
        {
            // A direct HelpAction clears parse errors. A regular action preserves input validation.
            command.SetAction(parseResult => new HelpAction().Invoke(parseResult));
        }

        foreach (var child in command.Subcommands)
        {
            ConfigureUsage(child);
        }
    }
    ...
    {
        var capture = new Command("capture", CaptureGuidance.Group);
    ...
    {
        var command = new Command(name, enabled ? CaptureGuidance.Start : CaptureGuidance.Stop);
        command.SetAction((parseResult, cancellationToken) =>
        {
            var json = parseResult.GetValue(jsonOption);
            return CliOutput.RunAsync(
                ...
                json,
                result => Console.WriteLine(result.IsProxyAttached
                    ? "Fiddler is attached as the Windows system proxy."
                    : "Fiddler is detached from the Windows system proxy. Explicitly routed traffic can still reach its listening proxy port."));
        });
        return command;
    }
    ...
    private static void WriteStatus(StatusResponse status)
    {
        ...
        Console.WriteLine($"System proxy:      {status.IsProxyAttached}");
