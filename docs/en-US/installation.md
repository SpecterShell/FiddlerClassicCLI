...
Keep the CLI host and bridge from the same build. An older daemon can reject **Save settings** with `Specify a bind mode, a port, or both.` when changing authentication alone. Update both components, stop the old daemon, and restart Fiddler. The panel checks daemon capabilities during refresh and before changes, disables settings after a failed status check, and preserves pending edits until the connection recovers.
...
The **Fiddler Classic CLI** tab and its Tools menu shortcut manage MCP HTTP access separately from Fiddler's capture proxy. The pane has **MCP**, **Named pipes**, and **Settings** subtabs. Bridge installation does not install Fiddler or change the system proxy, certificates, or HTTPS decryption. `bridge uninstall` removes the extension files and launch record after confirmation.

In **MCP**, the **Enable MCP HTTP** checkbox appears above Bind and Port. These controls remain editable while enabled, and edits take effect only after **Apply**. Applying changed bindings stops, configures, and restarts an enabled listener. Active connections, remote access, or mode `none` require confirmation before this sequence. A failed step stops the sequence and displays its error. The **MCP addresses** box below Apply/Refresh groups loopback and LAN URLs. **Authorized clients** and **Active connections** have separate boxes. Bind offers `loopback`, `all`, and `selected`. Selected mode shows a checklist for up to 16 active local IPv4 addresses. Saved addresses remain fixed. An unavailable selected address causes a bind error without a fallback to other interfaces.

Each displayed loopback or LAN URL has a clipboard-icon copy button immediately beside it. URLs and pipe addresses are selectable with the mouse or keyboard. Long URLs wrap as the pane narrows. When space is limited, copy buttons show just the icon and retain their tooltip and keyboard shortcut. After a successful copy, the clicked button shows **Copied!** for about 1.5 seconds, replacing the icon in compact mode. Its width fits the current caption and returns to its original width afterward. Button tooltips explain each action, including its effect on connections or credentials. The bind dropdown fits its choices using the font Windows actually renders. The selected-interface checklist uses `UseCompatibleTextRendering=false` to match surrounding native controls.

In **Settings**, choose a startup policy of `enabled`, `disabled`, or `last-state`. The default `last-state` preserves the saved desired state and leaves a new installation disabled. The policy applies at daemon startup and when Fiddler loads the extension, even if the daemon is already running. Saving a policy leaves the current listener state unchanged. The **Versions** box displays component versions, and **Documentation** links to the project and its documentation.

The authentication dropdown offers **Require for all** (`required`), **Non-loopback only** (`non-loopback`, the default), and **No authentication** (`none`). Changing it requires the service to be disabled. Fresh or unset configuration uses `non-loopback`. Explicit legacy config-file `HttpRequireAuthentication` values load as `required` for `true` and `none` for `false`. The exemption checks both actual socket IPs after normalizing IPv4-mapped addresses. LAN-address requests still require credentials even when sent locally, and unknown addresses or forwarded headers cannot grant an exemption. Saving or enabling loopback-only `non-loopback` needs no access-risk warning. Saving or enabling `none` requires confirmation that every reachable client will gain full MCP access, including captured traffic and mutation tools. Remote access with `required` or `non-loopback` also requires confirmation because tokens travel over plain HTTP. Local processes, including those under other Windows accounts, can use anonymous loopback MCP in the default `non-loopback` mode. Named pipes remain current-user-only. Local relays or reverse proxies connecting through loopback appear as loopback peers. Choose `required` if loopback callers or relays must authenticate. These settings apply only to the managed listener. Foreground `mcp http` remains authenticated and loopback-only. See [managed HTTP](cli.md#managed-mcp-http) for startup confirmation rules, CLI options, and security limits.
...
The read-only **Named pipes** subtab displays its controls directly, without a surrounding group box. It shows the full bridge and daemon paths as `\\.\pipe\<name>`, each with a copy button immediately to its right. These buttons use the same tooltips, compact layout, and **Copied!** feedback as the URL copy buttons. The subtab also lists the supported bridge protocol (v3), daemon protocol (v1), and the access restriction **Current Windows user only**. Windows ACLs enforce this restriction.

Bridge status reflects the local listener lifecycle: `Stopped`, `Starting`, `Listening`, `Retrying`, or `Unavailable`. Check the displayed last listener error if it cannot listen. A successful daemon status check shows `Responding` and its PID. A timeout or unavailable response leaves the daemon's state unknown and clears the PID to `Unknown`. A failed check alone cannot establish that the daemon has stopped. Check the host installation with `doctor` and retry the status check.
