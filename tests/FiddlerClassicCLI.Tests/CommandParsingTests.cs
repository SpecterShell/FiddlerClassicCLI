...
{
    [Fact]
    public void RootInstallIsRejectedWhileBridgeInstallRemainsAvailable()
    {
        var root = CreateRoot();
        Assert.DoesNotContain(root.Subcommands, command => command.Name == "install");
        Assert.NotEmpty(root.Parse("install").Errors);
        Assert.Empty(root.Parse("bridge install --json").Errors);
    }
    ...
    /// <summary>Checks every value-taking option can report a missing value without a validator throwing.</summary>
    [Fact]
    public void MissingOptionValuesReturnParserErrorsAcrossTheCommandTree()
    {
        var root = CreateRoot();
        CheckCommand(root, []);

        void CheckCommand(System.CommandLine.Command command, string[] path)
        {
            foreach (var option in command.Options.Where(option => option.Arity.MinimumNumberOfValues > 0))
            {
                string[] arguments = [.. path, option.Name];
                var result = root.Parse(arguments);
                Assert.True(result.Errors.Count > 0, $"Expected a missing-value error for {string.Join(' ', arguments)}.");
            }
            foreach (var child in command.Subcommands)
            {
                CheckCommand(child, [.. path, child.Name]);
            }
        }
    }

    [Theory]
    [InlineData(FiddlerClassicCLI.Protocol.ErrorCodes.Unavailable, ExitCodes.Unavailable)]
    [InlineData(FiddlerClassicCLI.Protocol.ErrorCodes.Timeout, ExitCodes.Timeout)]
    [InlineData(FiddlerClassicCLI.Protocol.ErrorCodes.NotFound, ExitCodes.Rejected)]
    [InlineData(FiddlerClassicCLI.Protocol.ErrorCodes.InvalidRequest, ExitCodes.Usage)]
