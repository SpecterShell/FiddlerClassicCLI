...
public sealed class McpToolTests
{
    ...
    /// <summary>Checks shared guidance and attachment forwarding through a fake bridge without daemon or proxy access.</summary>
    /// <param name="methodName">The capture tool method to inspect.</param>
    /// <param name="description">The shared description expected on the tool.</param>
    /// <param name="enabled">The requested system proxy attachment state.</param>
    [Theory]
    [InlineData(nameof(FiddlerTools.StartCapture), CaptureGuidance.Start, true)]
    [InlineData(nameof(FiddlerTools.StopCapture), CaptureGuidance.Stop, false)]
    public async Task CaptureToolsShareGuidanceAndForwardAttachmentOnly(string methodName, string description, bool enabled)
    {
        var method = typeof(FiddlerTools).GetMethod(methodName)!;
        Assert.Equal(description, method.GetCustomAttribute<DescriptionAttribute>()!.Description);
        ...
        var bridge = new TestBridgeClient
        {
            ...
            {
                calls++;
                Assert.Equal(Operations.SetCapture, operation);
                Assert.Equal(enabled, Assert.IsType<SetCaptureRequest>(request).Enabled);
                return new CaptureResponse { IsProxyAttached = enabled };
            }
        };
        var tools = CreateTools(bridge);
        var response = enabled
            ? await tools.StartCapture(TestContext.Current.CancellationToken)
            : await tools.StopCapture(TestContext.Current.CancellationToken);

        Assert.Equal(enabled, response.IsProxyAttached);
