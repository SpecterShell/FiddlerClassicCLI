// Checks the shipped extension inside a disposable native Fiddler process; never shipped with the CLI.
...
namespace FiddlerClassic.CompatibilityProbe;
...
public sealed class CompatibilityProbe : IFiddlerExtension
{
    ...
    private static IFiddlerExtension FindBridge()
    {
        ...
        var loaded = typeof(FiddlerExtensions).GetFields(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public)
            ...
            .Where(extension => extension.GetType().FullName == "FiddlerClassic.Bridge.BridgeExtension").Distinct().ToArray();
