...
public sealed partial class BridgeControlPanelTests
{
    ...
    public void CopiesUseClientUrlsAndRemainAvailableDuringRefresh(bool enabled) => RunOnSta(() =>
    {
        ...
        Assert.False(Button(panel, "Copy loopback").Enabled);
        ...
        Assert.True(Button(panel, "Copy loopback").Enabled);
    ...
    public void ControlsHaveExplicitNamesAndLogicalKeyboardOrder() => RunOnSta(() =>
    {
        ...
        Assert.Equal(new[]
        {
            "Listener bind mode", "Listener port", "Apply listener settings", "Enable MCP HTTP service",
            "Copy loopback endpoint", "Refresh service status", "Copy LAN endpoint 1", "Copy LAN endpoint 2",
        ...
        Assert.All(focusable.OfType<Button>(), button => Assert.Contains("&", button.Text));
